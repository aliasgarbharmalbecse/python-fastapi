...
from schemas.department_schema import DepartmentRequest, DepartmentUpdateRequest, DepartmentResponse
...
async def create_department(department: DepartmentRequest, db: Session = Depends(get_db)):
    try:
...
async def update_department(department: DepartmentUpdateRequest, db: Session = Depends(get_db)):
    dept_repo = DepartmentRepository(db)
